...
output = None
running = True
recording_lock = threading.Lock()  # Lock for recording operations
key_pressed = None
...
# Glove speed tracking for threshold check
glove_position_history = []  # List of (x, y, timestamp) tuples
MAX_HISTORY_LENGTH = 5  # Keep last 5 positions for velocity calculation
...
# ============================================================================
# Glove Speed Tracking Functions
# ============================================================================
def update_glove_position(glove_center, timestamp=None):
    """Update glove position history for velocity calculation"""
    global glove_position_history
    if timestamp is None:
        timestamp = time.time()
    
    with velocity_lock:
        glove_position_history.append((glove_center[0], glove_center[1], timestamp))
        # Keep only recent history
        if len(glove_position_history) > MAX_HISTORY_LENGTH:
            glove_position_history.pop(0)

def calculate_glove_speed():
    """
    Calculate glove speed in pixels per second
    Returns: speed in pixels/second (float)
    """
    global glove_position_history
    
    with velocity_lock:
        if len(glove_position_history) < 2:
            return 0.0
        
        # Use most recent and a few frames back for stable velocity
        recent = glove_position_history[-1]
        # Use position from 2-3 frames ago for more stable calculation
        if len(glove_position_history) >= 3:
            older = glove_position_history[-3]
        else:
            older = glove_position_history[0]
        ...
        # Calculate speed magnitude (pixels per second)
...
def extract_cv_data(detections, labels, glove_category, pixel_scale):
    ...
    Returns: (handConf, object_class, objectConf, distance_mm, glove_speed)
    ...
    obj_conf = None
    distance_mm = None
    glove_speed = 0.0  # pixels per second
    
    if not detections:
        return hand_conf, obj_class, obj_conf, distance_mm, glove_speed
    ...
    for detection in detections:
        if glove_category is not None and int(detection.category) == glove_category:
            ...
            update_glove_position(glove_center, current_time)
            # Calculate current speed
            glove_speed = calculate_glove_speed()
            break
...
def get_args():
    ...
    parser.add_argument("--speed-threshold", type=float, default=100.0,
                        help="Maximum glove speed (pixels/sec) to allow triggering. "
                             "If speed exceeds this, signal is not sent (default: 100.0 px/s)")
...
if __name__ == "__main__":
    ...
    if not args.no_serial:
        ...
        print(f"   Speed threshold: {args.speed_threshold:.1f} px/s (signals blocked if exceeded)")
        print("="*60)
    print("⌨️  Controls:")
        ...
        while running:
            ...
            if not args.no_serial and serial_port_obj:
                ...
                if current_time - last_cv_send_time >= cv_send_interval:
                    ...
                    # Extract CV data from detections (now includes speed)
                    hand_conf, obj_class, obj_conf, distance_mm, glove_speed = extract_cv_data(
                        last_results, labels, glove_category, args.pixel_scale
                    )
                    
                    # Check speed threshold - don't send if moving too fast
                    speed_threshold = args.speed_threshold  # pixels per second
                    
                    if glove_speed > speed_threshold:
                        # Moving too fast - don't trigger/send signal
                        if args.debug_cv:
                            print(f"[Speed Threshold] Speed {glove_speed:.1f} px/s > threshold {speed_threshold:.1f} px/s - NOT sending signal")
                        # Skip sending signal when moving too fast
                    else:
                        # Speed is acceptable - send the signal
                        if args.debug_cv:
                            ...
                            print(f"[CV Debug] Glove speed: {glove_speed:.1f} px/s (threshold: {speed_threshold:.1f} px/s)")
                ...
                if key == 'q' or key == '\x1b':  # 'q' or ESC
                    ...
                    with recording_lock:
                        if not recording:
                            ...
                            # Stop recording - do it outside the lock to avoid blocking
                            recording = False  # Set flag first
                            print(f"⏹️  Stopping recording...")
                    
                    # Stop recording outside lock to minimize blocking time
                    try:
